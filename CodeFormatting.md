...
* [前端代码规范](#前端代码规范)
  ...
  *  1.6 [SASS规范](#sass规范)
     ...
     * 1.6.4 [嵌套规范](#嵌套规范)
       ...
       * 1.6.4.2 [属性嵌套](#属性嵌套)
     * 1.6.5 [变量](#变量)
     * 1.6.6 [混合(mixin)](#混合mixin)
     * 1.6.7 [占位选择器 %](#占位选择器-)
     * 1.6.8 [extend 继承](#extend-继承)
     * 1.6.9 [for 循环](#for-循环)
     * 1.6.10 [each 循环](#each-循环)
...
* 技术栈选型由采用架构决定   MVVM  MVC
* MVVM 架构下  使用 vue 搭配 UI组件库
* UI 组件库以  `element-ui` 为主,`但鉴于近期 element-ui 更新维护频率下降,存在无法及时维护原因`. 选择 `antdv` 为辅.
* vue 3 已进入rc 阶段,正式版即将发布.   因此 `vue3` 下  UI组件库  `antdv`  为主.   `vue2`  下根据项目实际情况使用 `element-ui\antdv ` 均可
* 

# 基本要求

* 开发中代码以精简为主,用最少的代码实现各种效果.
* 一些简单效果建议使用伪类实现( ::before \ ::after ),类似选中效果的边线\一些不规则的分割线等
* 

```
未方便进行屏幕适配,字体大小位置等统一建议: 
...
为方便字号单位转换
   ...
   body 统一指定字号        字行高默认指定为字号1.5倍 
   
    font-size:10px;            font:100%/1.5em; 
    

导航文字  :16\18         1.6\1.8rem   
...
正文      :12\14         1.2\1.8rem        #666

提示性文字:12\14         1.2\1.4rem        #999  #ddd  #eee
```

#### 方法命名 
* getFunction()  获取   `table 表格数据 以 data   结尾,  列表数据(select等)以  list  结尾, 树形解构以 tree 结尾 `
* addFunction()   添加   
* updateFunction()  修改
* deleteFunction()\delFunction()   删除
